...
function recordLoginFailure() {
  ...
  if (loginAttempts.count >= 5) {
    loginAttempts.lockedUntil = Date.now() + 60 * 1000; // 60s lockout after 5 failures
  }
}
...
export default function App() {
  const [screen, setScreen] = useState('loading'); // loading | welcome | register | login | app
  ...
  useEffect(() => {
    (async () => {
      ...
      if (savedCode && savedKeys) {
        ...
        setScreen('login'); // existing user → go straight to login
      } else {
        setScreen('welcome'); // new user → show welcome/warning page first
      }
  ...
  const connectSocket = useCallback((token) => {
    ...
    socket.on('typing_start', ({ from }) => {
      if (activeChat?.accountCode === from) setMascotMood(MOODS.TYPING);
    });
  ...
  const handleIncomingMessage = useCallback(async (envelope) => {
    ...
    if (activeChat?.accountCode === envelope.from) setMessages(prev => [...prev, msg]);
  ...
  const handleIncomingVoice = useCallback(async (envelope) => {
    ...
    if (activeChat?.accountCode === envelope.from) setMessages(prev => [...prev, voiceMsg]);
  }, [keys, activeChat]);
  ...
  const handleLogin = async (code, password) => {
    setMascotMood(MOODS.TYPING);
    try {
      checkLoginRateLimit(); // throws if locked
      ...
      // Rate-limit error — just show it, don't record another failure
  ...
  const handleSendMessage = async () => {
    ...
    const encrypted = encryptMessage(
      ...
      contactPubKey.publicKey, keys.secretKey
    );
    const msgId = crypto.randomUUID();
    const msg = {
      id: msgId, from: accountCode, content: inputText.trim(),
      selfDestructSeconds: selfDestructTime,
      selfDestructAt: selfDestructTime ? Date.now() + selfDestructTime * 1000 : null,
      ts: Date.now(), status: 'sent', conversationId: activeChat.accountCode,
    };
    ...
    try {
      socketRef.current?.emit('private_message', {
        id: msgId, recipientCode: activeChat.accountCode,
  ...
  const handleRecallMessage = async (msgId) => {
    ...
    socketRef.current?.emit('recall_message', { messageId: msgId, recipientCode: activeChat.accountCode });
  ...
  const sendVoiceMessage = async (blob) => {
    ...
    socketRef.current?.emit('voice_message', {
      id: crypto.randomUUID(), recipientCode: activeChat.accountCode, payload: encrypted, duration: 0,
    });
    await apiSendVoice(activeChat.accountCode, encrypted, 0);
  };
  ...
  if (screen === 'welcome') {
    return <WelcomeScreen onContinue={() => setScreen('register')} onLogin={() => setScreen('login')} />;
  }
  ...
  if (screen === 'login') {
    return (
      <LoginScreen
        onLogin={handleLogin}
        onSwitchToRegister={() => setScreen('welcome')}
...
const WELCOME_CSS = `
  ...
  .wc-inner {
    ...
    align-items: center;
    animation: wc-rise 0.55s cubic-bezier(0.16,1,0.3,1) both;
  }
  ...
  .wc-ghost {
    ...
    filter: drop-shadow(0 0 32px rgba(130,100,255,0.45));
    animation: wc-float 3.5s ease-in-out infinite;
  }
  @keyframes wc-float {
  ...
  .wc-logo {
    ...
    color: #fff;
    margin: 0 0 0.25rem;
    text-shadow: 0 0 40px rgba(130,100,255,0.6);
  }
  ...
  .wc-warn-title {
    ...
    margin: 0 0 0.4rem;
    text-align: center;
    animation: wc-rise 0.4s 0.05s both;
  }
  ...
  .wc-warn-sub {
    ...
    margin: 0 0 1.8rem;
    text-align: center;
    animation: wc-rise 0.4s 0.1s both;
  }
  ...
  .wc-btn-primary {
    width: 100%;
    padding: 0.9rem;
    background: linear-gradient(135deg, #6c3de8, #9b5cf6);
    border: none;
    border-radius: 12px;
    color: #fff;
    font-family: 'DM Sans', sans-serif;
    ...
    cursor: pointer;
    box-shadow: 0 4px 28px rgba(108,61,232,0.45);
...
function WelcomeScreen({ onContinue, onLogin }) {
  const [step, setStep] = useState(0); // 0 = intro, 1 = warnings
  ...
  const features = [
    ...
    { icon: '💣', name: 'Self-destruct messages', desc: 'Set timers. Messages disappear automatically after reading.' },
    { icon: '☠', name: 'Panic wipe', desc: 'One button instantly destroys all your data on this device.' },
    { icon: '👻', name: 'Ghost mode', desc: 'Appear offline while still receiving messages.' },
    { icon: '💾', name: 'Local storage only', desc: 'Nothing is ever uploaded to a server. Your device, your data.' },
  ];
  ...
  const warnings = [
    {
      ...
      body: 'There is no email reset, no support ticket, no way back. Forget your account code or password and your account is gone forever. Write them down somewhere safe.',
    },
    {
      icon: '📱',
      title: 'Messages live on this device only',
      body: 'All messages are stored in your browser. Clearing browser data, switching devices, or uninstalling wipes everything permanently. No cloud backup exists.',
    },
    {
      icon: '👻',
      title: 'Ephemeral by design',
      body: 'Self-destruct timers and panic wipe are permanent. Burned messages cannot be recovered by anyone, including us. Do not use Ghost Chat as an archive.',
    },
    {
      icon: '🚫',
      title: 'No moderation safety net',
      body: 'Ghost Chat is fully anonymous. We cannot identify, ban, or contact users. Do not share sensitive personal information. Exercise caution with strangers.',
    },
  ];
    ...
    <>
      ...
      <div className="wc-page">
        <div className="wc-inner">
          {step === 0 ? (
            <>
              <div className="wc-ghost">
                <GhostMascot mood={MOODS.IDLE} size={96} />
              </div>
              ...
              <p className="wc-desc">
                Ghost Chat is a zero-knowledge anonymous messenger. Your messages never touch our servers in readable form. Your identity is a random cryptographic code. Nothing is logged. Nothing is tracked.
              </p>

              <button className="wc-btn-primary" onClick={() => setStep(1)}>
                Continue — read the warnings →
              </button>
              <button className="wc-btn-ghost" onClick={onLogin}>
            ...
            <>
              ...
              <div className="wc-warn-list">
                {warnings.map((w, i) => (
                  <div className="wc-warn-item" key={i} style={{ animationDelay: `${i * 0.07}s` }}>
              ...
              <button className="wc-btn-primary" onClick={onContinue}>
                I understand — create my account
              </button>
              <button className="wc-back-link" onClick={() => setStep(0)}>
                ← Go back
              </button>
            </>
          )}
...
function LockScreen({ onUnlock, mascotMood }) {
  ...
  return (
    <div className="screen center">
      ...
      <input
        type="password" placeholder="Enter password to unlock"
...
// ── Login screen with rate-limit UI ───────────────────────────────────────────
function LoginScreen({ onLogin, onSwitchToRegister, mascotMood, savedAccountCode }) {
  ...
  const handleSubmit = async () => {
    ...
    // Reflect updated attempt count in local error after onLogin resolves
    if (loginAttempts.lockedUntil) {
      // timer useEffect will pick it up
    } else if (loginAttempts.count > 0) {
      const remaining = 5 - loginAttempts.count;
      setLocalErr(`Invalid credentials. ${remaining} attempt${remaining === 1 ? '' : 's'} remaining before lockout.`);
    }
    setLoading(false);
  };

  const isLocked = countdown > 0;
  const attemptDots = loginAttempts.count > 0 && !isLocked;
      ...
      <div className="form">
        ...
        {localErr && (
          <div style={{
            ...
            background: isLocked ? 'rgba(255,50,50,0.1)' : 'rgba(255,120,50,0.08)',
            border: `1px solid ${isLocked ? 'rgba(255,50,50,0.3)' : 'rgba(255,120,50,0.2)'}`,
            borderRadius: 9, display: 'flex', alignItems: 'center', gap: 8,
          }}>
            <span style={{ fontSize: 15 }}>{isLocked ? '🔒' : '⚠️'}</span>
            <span style={{ color: isLocked ? '#ff8080' : '#ffaa70', fontSize: 12, fontFamily: 'monospace' }}>
              {localErr}
            </span>
          </div>
        )}

        {/* Attempt indicator dots (5 dots, fill red as attempts increase) */}
        {attemptDots && (
          <div style={{ display: 'flex', gap: 5, justifyContent: 'center', marginTop: 10 }}>
            {[...Array(5)].map((_, i) => (
              <div key={i} style={{
                width: 8, height: 8, borderRadius: '50%',
                background: i < loginAttempts.count ? '#ff5555' : 'rgba(255,255,255,0.12)',
                transition: 'background 0.3s',
                boxShadow: i < loginAttempts.count ? '0 0 6px rgba(255,50,50,0.6)' : 'none',
              }} />
            ))}
          </div>
        )}
...
// ────────────────────────────────────────────────────────────────────────────
// Chat App (unchanged from previous version)
// ────────────────────────────────────────────────────────────────────────────
          ...
          <div className="friend-list">
            ...
            {showAddFriend && (
              <div style={{ padding: '0 12px 12px' }}>
                <input
                  className="input" placeholder="Friend code"
                  value={addFriendCode} onChange={e => setAddFriendCode(e.target.value)}
                  style={{ fontSize: 13 }}
                />
                <button
                  className="btn-primary"
...
function SettingsPanel({
  ghostMode, onToggleGhostMode, minimalMode, onToggleMinimal, onLogout,
      ...
      <div style={{ marginTop: 20 }}>
        ...
        {showDecoySetup && (
          <div>
            ...
            <button className="btn-primary" style={{ marginTop: 8, width: '100%' }} onClick={onSaveDecoy}>
              Save Decoy
            </button>
          </div>
        )}
